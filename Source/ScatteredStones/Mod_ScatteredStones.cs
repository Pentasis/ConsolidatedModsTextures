...
namespace ConsolidatedMods.Textures
{
	public class Mod_ScatteredStones : Mod
	{
		...
		public override void DoSettingsWindowContents(Rect inRect)
		{
			...
			options.Label("ScatteredStones.Settings.ScaleSliderMin".Translate("1", "0.5", "2") + Math.Round(minScaleModifier, 2), -1f, tipSignal);
			minScaleModifier = options.Slider(minScaleModifier, 0.5f, 2f);

			options.Label("ScatteredStones.Settings.ScaleSliderMax".Translate("1", "0.5", "2") + Math.Round(maxScaleModifier, 2), -1f, tipSignal);
			maxScaleModifier = options.Slider(maxScaleModifier, 0.5f, 2f);

			options.Label("ScatteredStones.Settings.OffsetSlider".Translate("1", "0", "3") + Math.Round(offsetModifier, 2), -1f, tipSignal);

	...
	public class Patch_SpawnSetup
	{
		static public void Postfix(Map map, Building __instance)
		{
			...
			if (__instance.def.passability != Traversability.Standable && map?.AgeInDays != 0 && __instance.positionInt != IntVec3.Invalid)
			{
				...
				{
					var cell = list[i];
					if (cell.InBounds(map) && !cell.Standable(map)) Utility.ValidateCell(cell, map, true);
				}
			}
		}
    }
    ...
    public class Patch_TrySpawnYield
	{
		static public void Postfix(Mineable __instance, Map map)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				{
					RockFilth rockFilth = ThingMaker.MakeThing(Owl_Filth_Rocks, null) as RockFilth;
					//Place underneath chunk
					GenPlace.TryPlaceThing(rockFilth, __instance.Position, map, ThingPlaceMode.Direct);
					//Match color
					rockFilth.DrawColor = rockFilth.MatchColor(__instance);
				}
			}
		}
	}
